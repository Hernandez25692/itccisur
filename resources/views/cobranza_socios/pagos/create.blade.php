    ...
    <div class="max-w-5xl mx-auto py-8 px-4 space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500">{{ $empresa->nombre_empresa }} · RTN {{ $empresa->rtn_empresa }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('cobranza.pagos.store') }}"
                ...
                <div>
                    ...
                    <select name="metodo" class="w-full rounded-xl border-gray-300">
                        @foreach (['efectivo', 'transferencia', 'deposito', 'cheque', 'otro'] as $m)
                            <option value="{{ $m }}" @selected(old('metodo', 'efectivo') == $m)>{{ strtoupper($m) }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="text-sm font-semibold text-gray-700">Gestor (user_id opcional)</label>
                    <input name="gestor_id" value="{{ old('gestor_id') }}" class="w-full rounded-xl border-gray-300"
            ...
            <div class="bg-gray-50 border rounded-2xl p-4">
                <p class="font-semibold text-gray-900">Cargos pendientes (aplicación FIFO automática)</p>
                <p class="text-sm text-gray-500">El pago se aplicará primero al cargo más antiguo pendiente.</p>
                <div class="mt-3 overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y">
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="py-3 text-gray-400">No hay cargos pendientes.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="flex items-center justify-end gap-2">
                ...
                <button class="px-5 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700">Registrar pago</button>
            </div>
        </form>
    </div>
</x-app-layout>
