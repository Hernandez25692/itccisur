        ...
        <div class="mb-8">
            ...
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    ...
                    <p class="text-gray-600 mt-1">Agrega una nueva actividad al Plan de Trabajo TI {{ $plan->anio }}
                    </p>
                </div>
            </div>
        </div>
            ...
            <form action="{{ route('plan-actividad.store', $plan->id) }}" method="POST" class="p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <select name="seccion"
                            ...
                            <option value="Infraestructura y Seguridad"
                                {{ old('seccion') == 'Infraestructura y Seguridad' ? 'selected' : '' }}>Infraestructura
                                y Seguridad</option>
                            <option value="Redes y Comunicaciones"
                                {{ old('seccion') == 'Redes y Comunicaciones' ? 'selected' : '' }}>Redes y
                                Comunicaciones</option>
                            <option value="Soporte Técnico" {{ old('seccion') == 'Soporte Técnico' ? 'selected' : '' }}>
                                Soporte Técnico</option>
                            <option value="Seguridad Informática"
                                {{ old('seccion') == 'Seguridad Informática' ? 'selected' : '' }}>Seguridad Informática
                            </option>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="actividad" value="{{ old('actividad') }}"
                            ...
                            placeholder="Ej: Actualización de servidores Windows Server" required>
                        @error('actividad')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="objetivo" rows="3"
                            ...
                            placeholder="Describe el objetivo específico de esta actividad..." required>{{ old('objetivo') }}</textarea>
                        @error('objetivo')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="frecuencia" value="{{ old('frecuencia') }}"
                            ...
                            placeholder="Ej: Anual, Semestral, Mensual, Trimestral" required>
                        @error('frecuencia')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="responsable" value="{{ old('responsable') }}"
                            ...
                            placeholder="Ej: Juan Pérez, María García" required>
                        @error('responsable')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="mes_previsto"
                            ...
                            <option value="Enero" {{ old('mes_previsto') == 'Enero' ? 'selected' : '' }}>Enero
                            </option>
                            <option value="Febrero" {{ old('mes_previsto') == 'Febrero' ? 'selected' : '' }}>Febrero
                            </option>
                            <option value="Marzo" {{ old('mes_previsto') == 'Marzo' ? 'selected' : '' }}>Marzo
                            </option>
                            <option value="Abril" {{ old('mes_previsto') == 'Abril' ? 'selected' : '' }}>Abril
                            </option>
                            <option value="Mayo" {{ old('mes_previsto') == 'Mayo' ? 'selected' : '' }}>Mayo</option>
                            <option value="Junio" {{ old('mes_previsto') == 'Junio' ? 'selected' : '' }}>Junio
                            </option>
                            <option value="Julio" {{ old('mes_previsto') == 'Julio' ? 'selected' : '' }}>Julio
                            </option>
                            <option value="Agosto" {{ old('mes_previsto') == 'Agosto' ? 'selected' : '' }}>Agosto
                            </option>
                            <option value="Septiembre" {{ old('mes_previsto') == 'Septiembre' ? 'selected' : '' }}>
                                Septiembre</option>
                            <option value="Octubre" {{ old('mes_previsto') == 'Octubre' ? 'selected' : '' }}>Octubre
                            </option>
                            <option value="Noviembre" {{ old('mes_previsto') == 'Noviembre' ? 'selected' : '' }}>
                                Noviembre</option>
                            <option value="Diciembre" {{ old('mes_previsto') == 'Diciembre' ? 'selected' : '' }}>
                                Diciembre</option>
                        </select>
                        @error('mes_previsto')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="fecha_ejecucion" value="{{ old('fecha_ejecucion') }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="metrica_exito" value="{{ old('metrica_exito') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="flex items-start gap-3">
                    <div class="p-2 bg-white rounded-lg shadow-sm">
                        <svg class="w-5 h-5 text-[#C5A049]" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Usa un sistema de numeración jerárquico como 1.1, 1.2, 2.1.1
                            para organizar las actividades.</p>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Define objetivos específicos y métricas claras para evaluar el
                            éxito de cada actividad.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        if (frecuenciaInput) {
            frecuenciaInput.addEventListener('focus', function() {
                const suggestions = ['Anual', 'Semestral', 'Trimestral', 'Mensual', 'Semanal', 'Única vez',
                    'Según demanda'
                ];
